...
var exwModule = {
	...
	initialize: function(ide){
		...
		if(!toolsFolder.exists()){
			fs.mkdirSync(toolsFolder.getPath(), {recursive: true});
		}
		...
		ide.emitter.on('did-doc-saved', (doc) => {
			console.log("tools saved files %s", doc.title);
			console.log(this.tools.hooks.onSave);
			for(let uid of this.tools.hooks.onSave){
				let tool = this.tools.items.get(uid);
				console.log("onSave Hook for %s %o", uid, tool);

				toolRunWithUID(uid, "onSave", {doc: doc});
			}

		});

//#MARK COMMAND exw-ide:reload-tools
		ide.addCommand('exw-ide:reload-tools', 'Reload tools', () => {
			this.tools.items = new Map();
			this.loadTools();
		});
	},
...
function toolRunWithUID(uid, hook, ctxData){
	...
	var ctx = ide.controller.getScriptContext();
	if(ctxData){
		for(let key of Object.keys(ctxData)){
			ctx[key] = ctxData[key];
		}
	}
...
function updateToolsTree(){
	...
	let createTool = (tool) => {
		...
		listItemName.setAttribute('data-tool-path', tool.__source_path);
