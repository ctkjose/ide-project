...
var initializeIDE = function(){
	...
	if (!atom.packages.isPackageLoaded('atom-ide-ui')) {
		atom.notifications.addWarning(
			'ide-project package requires the package atom-ide-ui',
			{
				detail: 'Install atom-ide-ui and restart your editor.'
			}
		)
	}
...
//#MARK GET ATOM-IDE-UI services
exwController.consumeConsole = (createConsole) => {
	ide.console = createConsole({id: 'exw-ide', name: 'EXW-IDE'});
	//ide.console.log('A message!');
	//ide.console.error('A test error');
	return new Disposable(() => { ide.console = null });
};
exwController.consumeBusySignal = (busySignal) => {
	ide.busySignal = busySignal;
	//https://github.com/facebookarchive/atom-ide-ui/blob/master/modules/atom-ide-ui/pkg/atom-ide-busy-signal/lib/types.js
	//ide.busySignal.reportBusy(msg, {onlyForFile:"URI", waitingFor: 'computer' | 'user', revealTooltip: false, onDidClick: null})

	ide.addBusySignal = (msg) => {

		let busyPromiseResolve = null;
		var out = {
			clear: () => {
				busyPromiseResolve();
			}
		...
		out.promise = new Promise((resolve, reject)=>{
			busyPromiseResolve = resolve;
		});

		ide.busySignal.reportBusyWhile(msg, () => out.promise, {waitingFor: 'computer', revealTooltip: true});
...
exwController.getScriptContext = function(){
	...
	ctx.ide = {
		...
		logError: (msg)=> {
			if(!ide.console) return;
			ide.console.error(msg);
		},
		logMessage: (msg)=> {
			if(!ide.console) return;
			ide.console.log(msg);
